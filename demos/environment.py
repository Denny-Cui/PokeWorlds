...
@click.option("--play_mode", type=click.Choice(["random", "restricted_random"]), default="random", help="Play mode: 'random' for random actions.")
...
def main(variant, init_state, play_mode, headless, save_video):
    ...
    environment = get_pokemon_environment(game_variant=variant, controller=controller, headless=headless, save_video=save_video, init_state=init_state)
    steps = 0
    max_steps = 500
