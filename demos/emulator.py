...
def main(variant, init_state, play_mode, headless, save_video):
    if play_mode == "human":
        emulator = get_pokemon_emulator(game_variant=variant, init_state=init_state, headless=False, save_video=save_video)
        emulator.human_play()
    else:
        if headless != False:
            headless = True
        emulator = get_pokemon_emulator(game_variant=variant, init_state=init_state, headless=headless, save_video=save_video)
