...
def main():
    ...
    if not os.path.exists("configs"):
        raise FileNotFoundError(
            "configs directory not found. Please run this script from the root of the project."
        )
    ...
    for key in all_params:
        ...
        if key == "env_dir" and not os.path.exists(all_params[key] + "/bin"):
            raise ValueError(
                f"Config variable {key} must be set to a virtual environment directory (but could not find {all_params[key]}/bin. Check that the path is correct."
            )
    if fail_keys:
        raise ValueError(
            f"The following keys have the value 'PLACEHOLDER'. Please set them in the config files before creating the env file: {fail_keys}"
        )
