...
class SimplePokemonExecutor(SimpleExecutor):
    ...
    def get_action_message(
        ...
        *,
        ...
        if action == MoveStepsAction or action == MoveGridAction:
            ...
            if action_success == -1:
                action_success_message = "There is nothing to interact from your current position, consider using the locate action to get the exact coordinates before moving to that spot and trying again."
            else:
                found_directions = {}
                for direction in ["up", "down", "left", "right"]:
                    if action_return[direction][1] == True:
                        found_directions[direction] = (
                            f"In the direction {direction}, there is {action_return[direction][0]}. To interact with this, first use the action `move({direction} 1)` and then use `interact()` again."
                        )
                action_success_message = f"There are things you may be able to interact with in certain directions from your current position.\n"
                for found_direction, found_desc in found_directions.items():
                    action_success_message += f"\t{found_desc}\n"
        elif action == PokemonLocateAction:
            if action_success == -1:
                action_success_message = (
                    f"Could not locate the target on screen. {locate_advice}"
                )
            else:
...
class EQAPokemonExecutor(EQAExecutor):
    ...
    def get_action_str(
        ...
        return PokemonExecutor.get_action_str(
            self,
            allowed_actions_str=allowed_actions_str,
            prev_action_strings=prev_action_strings,
    ...
    def get_action_message(
        ...
        *,
        ...
        action_success,
        action_return,
        last_action_hint=False,
    ):
        return PokemonExecutor.get_action_message(
            self,
            action=action,
