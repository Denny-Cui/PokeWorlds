...
class PokemonRedChooseCharmanderFastEnv(Environment):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Dict(
            {"facing": direction_space, "coords": coord_space}
        )
    ...
    def determine_reward(
        ...
        if starter_chosen is None:
            ...
            player_pos = current_state["pokemon_red_location"][
                "current_local_location"
            ][:2]
            # compute manhattan distance to closest starter spot
            dists = [
                abs(player_pos[0] - spot[0]) + abs(player_pos[1] - spot[1])
