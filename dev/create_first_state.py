...
@click.command()
@click.option("--game", type=click.Choice(AVAILABLE_GAMES), help="Variant of the Pokemon game to create the first state for.")
...
def create_first_state(game: str, state_name: str):
    ...
    if f"{game}_rom_data_path" not in parameters:
        log_error(f"ROM data path not found for game: {game}. Add {game}_rom_data_path to the config files. See configs/pokemon_red_vars.yaml for an example", parameters)
